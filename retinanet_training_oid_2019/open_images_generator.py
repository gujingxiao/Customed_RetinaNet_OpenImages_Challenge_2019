...
from labelProcess.label_levels import read_image_bgr_fast, OUTPUT_PATH

def get_labels(metadata_dir, version='v4', label_level=1):
    csv_file = 'class-descriptions-boxable-level-{}.csv'.format(label_level)
...
def get_image_sizes(subset):
    import pandas as pd
    if subset == 'validation' or subset == 'train':
        sizes = pd.read_csv(OUTPUT_PATH + 'train_image_params.csv')
    else:
...
def generate_images_annotations_json(main_dir, metadata_dir, subset, cls_index, label_level=1, version='v4'):
    validation_image_ids = {}

    if version == 'v4':
        annotations_path = os.path.join(metadata_dir, '{}-annotations-bbox-level-{}.csv'.format(subset, label_level))
    elif version == 'challenge2019':
        validation_image_ids_path = os.path.join(metadata_dir, 'val-exists-annotations-bbox-level-{}.csv'.format(label_level))
        ...
        if subset == 'train':
            annotations_path = os.path.join(metadata_dir, 'train-exists-annotations-bbox-level-{}.csv'.format(label_level))
        elif subset == 'validation':
            annotations_path = validation_image_ids_path
...
class OpenImagesGenerator(Generator):
    def __init__(
            self, main_dir, subset, version='v4', label_level=1,
        ...
        metadata_dir          = OUTPUT_PATH + 'level_{}_files/'.format(label_level)
        annotation_cache_json = os.path.join(metadata_dir, subset + '_level_{}.json'.format(label_level))

        self.id_to_labels, cls_index = get_labels(metadata_dir, version=version, label_level=label_level)
        ...
        if os.path.exists(annotation_cache_json):
            ...
            self.annotations = generate_images_annotations_json(main_dir, metadata_dir, subset, cls_index, label_level, version=version)
