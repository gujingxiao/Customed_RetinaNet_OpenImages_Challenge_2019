...
def create_strength_csv_for_retinanet(input_dir, mirror_dir, out_file, label_arr, skip_box_thr=0.05, intersection_thr=0.55, limit_boxes=300, type='avg'):
    out = open(out_file, 'w')
    out.write('ImageId,PredictionString\n')
    d1, d2 = get_description_for_labels()
    sample = ROOT_PATH + 'label/sample_submission.csv'
    ...
    for index in range(len(sampleid)):
        f = os.path.join(input_dir, sampleid[index] + '.pkl')
        mirror_f = os.path.join(mirror_dir, sampleid[index] + '.pkl')
        id = os.path.basename(f)[:-4]
        out.write(id + ',')
        if os.path.exists(f):
            if os.path.exists(mirror_f):
                boxes_m, scores_m, labels_m = load_from_file_fast(mirror_f)
            boxes, scores, labels = load_from_file_fast(f)

            boxes = np.concatenate((boxes, boxes_m), axis=0)
            scores = np.concatenate((scores, scores_m), axis=0)
...
if __name__ == '__main__':
    ...
    label_level = 4
    backbone = 'resnet50'
    pretrained_model_path = '../retinanet_training_oid_2019/resnet50_models/resnet50_oid_map_level_{}_04958.h5'.format(label_level)
    ...
    output_cache_mirror_directory = ROOT_PATH + 'cache_retinanet_level_{}_mirror_{}/'.format(label_level, backbone)
    create_strength_csv_for_retinanet(output_cache_directory, output_cache_mirror_directory, 'predictions_{}_{}_{}_level{}_strength.csv'.format(skip_box_confidence, iou_thr, type, label_level),
                                 labels_list, skip_box_confidence, iou_thr, limit_boxes_per_image, type=type)